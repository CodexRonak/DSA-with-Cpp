...
bool isPossible(int cook[], int n, int dish, int m){
    int totalDishes = 0;
    for (int i = 0; i < n; i++){
        int totalTime = 0;
        int dishCount = 0;
        ...
        while(totalTime+(multiplier * cookRank) <= m){
            totalTime += multiplier * cookRank;
            dishCount++;
            multiplier++;
        }
...
int minTime(int cook[], int n, int dish){
    int s = 0;
    int e = 10 * ((dish * (dish + 1) / 2));
    int ans = -1;

    while(s <= e){
        int m = s + (e - s) / 2;
        if(isPossible(cook, n, dish, m)){
            ans = m;
            e = m - 1;
        }
        else{
